...
Stop Network::getStopById(const std::string& stopId)    // definition der funktion "getStopById", die von einer stopId den namen der haltestelle bekommt
{
    ...
    {
        return positionfinder->second;          // da stops eine unordered map ist, ist "second" einfach der wert des zugehörigen schlüssels ("first"), also soll die position nun auf die haltestelle der stopId zeigen
    }
...
{
    std::vector<StopTime> passendeResultsZuTripUndQuery;
    ...
    for (const auto& stopTime : stopTimes)
    {
        if (stopTime.tripId == tripId)
        {
            ...
            if (stop.name.find(query) != std::string::npos)
            {
                passendeResultsZuTripUndQuery.push_back(stopTime);
            }
        }
    }

    return passendeResultsZuTripUndQuery;
}
