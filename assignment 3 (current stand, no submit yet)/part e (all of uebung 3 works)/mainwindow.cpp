    ...
    , beispiel("C:/Users/varun/OneDrive/Desktop/SoSe25CODE/ubung-3-abgabe-so-se-25/allfiles") // network-objekt bekommt "zugriff" auf die files via wertzuweisung des datenpfads
{
    ...
    ui->tableView->setEnabled(false);   // deaktivierung der tabelle (am anfang)
...
void MainWindow::SuchWortChanged()  // funktion definition für wenn sich das suchwort ändert --> hat jetzt einfluss auf anzeige der tabelle
{
    ...
    std::string suchwort_als_string = input.toStdString();  // da wir aber mit dem text mittels anderer funktion arbeiten wollen, konvertieren wir es in einen string und speichern es in "suchwort_als_string"

    int routeIndex = ui->comboBox->currentIndex();  // der index der user-gewählten linie/route wird bestimmt und in "routeIndex" gespeichert
    int tripIndex = ui->comboBox_trips->currentIndex(); // der index der user-gewählten fahrt/trip wird bestimmt und in "tripIndex" gespeichert

    if (routeIndex < 0 || tripIndex < 0)    // wenn irgendeiner der beiden eine index kleiner 0 ist (also keine linie/trip entspricht)
    {
        ui->tableView->setModel(nullptr);   // zeige keine daten in der tabelle und beende
        return;
    }

    bht::Route route = beispiel.getRoutes()[routeIndex];    // mittels der funktion "getRoutes" und der "routeIndex" wird die einzelne linie (in form des route-objekts) in "route" gespeichert
    ...
    if ((size_t)tripIndex >= trips.size())  // kurzer check, ob der tripIndex kleiner gleich ist als die anzahl an trips (e.g. tripindex = 19, bei 5 trips)
    {
        ui->tableView->setModel(nullptr);   // zeige keine daten in der tabelle und beende
        return;
    }

    bht::Trip selectedTrip = trips[tripIndex];  // mittels von "tripIndex" innerhalb des vectors "trips" wird der bestimmte trip ausgewählt und in "selectedTrip" gespeichert

    std::vector<bht::StopTime> filteredStopTimes = beispiel.searchStopTimesForTrip(suchwort_als_string, selectedTrip.id);   // jetzt die nutzung von "searchStopTimesForTrip", in der das suchwort und die "id" des ausgewählten trips die anzeige der tabelle beeinflussen
    ...
    for (int row = 0; row < (int)filteredStopTimes.size(); ++row)   // normale for schleife : durchgehen der haltestellen, die mit "suchwort" und "id" übereinstimmen
    {
        const auto& stopTime = filteredStopTimes[row];              //  einzelne zeile der gefilterten stoptimes wird in "stoptime" gespeichert
        bht::Stop stop = beispiel.getStopById(stopTime.stopId);     // dazu wird das entprechende "stop"-objekt mittels der stopId ermittelt

        model->setItem(row, 0, new QStandardItem(QString::number(stopTime.stopSequence)));      // die tabelle wird gefüllt
        model->setItem(row, 1, new QStandardItem(QString::fromStdString(stop.name)));
        model->setItem(row, 2, new QStandardItem(QString::fromStdString(TimeToStringConverter(stopTime.arrivalTime))));     // nutzung von "timetostringconverter" für die zeiten
