...
std::vector<bht::Stop> Network::getTravelPath(const std::string& fromStopId, const std::string& toStopId)   // verlangt methode für die berechnung des kürzesten weg von start bis ziel
{
    ...
    if (letzte.find(toStopId) == letzte.end() && fromStopId != toStopId) // wenn kein möglicher weg zustande kommt und wir nicht am ziel sind, dann gib einfach die leeren vektor aus
    {
        return std::vector<bht::Stop>();
    }
    ...
    while(aktuell != fromStopId)        // solange wir nicht beim start sind
    {
        ...
        aktuell = letzte[aktuell];          // und kehre zum vorgänger knoten zurück
    }
...
NetworkScheduledTrip Network::getScheduledTrip(const std::string& tripId) const // methode für übergabe einer tripId und rückgabe eines NetworkScheduledTrips
{
    ...
    return NetworkScheduledTrip(tripId, passendestoptimes);     // erzeugung und rückgabe des NetworkScheduledTrip objekts mit übergabe der tripId und den passenden stoptimes in reihenfolge (in fahrtrichtung)
