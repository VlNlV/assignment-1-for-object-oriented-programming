...
NetworkScheduledTrip::NetworkScheduledTrip(const Trip& ganzetripobjekt, const std::vector<StopTime>& stoptimesvector) : stoptimesvector(stoptimesvector), ganzetripobjekt(ganzetripobjekt){}    // konstruktor für das objekt "NetworkScheduledTrip" (mit initialisierung von einer tripID und eienr liste von stoptimes)

const std::string& NetworkScheduledTrip::tripIDgetter()const    // getter methode für tripID
{
  return ganzetripobjekt.id;  // returnt id des objekts
}
...
{
  return gezeigtertrip->ganzetripobjekt.id;   // returnt die tripID des iterators
}

unsigned int NetworkScheduledTrip::iterator::getStopSequence()const
{
    return index;      // returnt den index
}

NetworkScheduledTrip::iterator NetworkScheduledTrip::at(const std::string& stopId) const {
    for (size_t index = 0; index < stoptimesvector.size(); ++index) {
        if (stoptimesvector[index].stopId == stopId) {
            return iterator(this, static_cast<int>(index));
        }
    }
    return end();
}
