...
void Network::TripsLoader(std::string path) {       // ähnliches wie oben! siehe "agencyloader"
    ...
    while (reader.next()) {
        ...
        auto range = tripzustoptimes.equal_range(trip.id);
        for (auto iterator = range.first; iterator != range.second; ++iterator) 
        {
            selectedStopTimes.push_back(iterator->second);
        }
        std::sort(selectedStopTimes.begin(), selectedStopTimes.end());
        std::shared_ptr<NetworkScheduledTrip> scheduledTrip{new NetworkScheduledTrip(trip, selectedStopTimes)};
        tripsdiescheduledsind[trip.id] = scheduledTrip;
    }
}
...
{
    std::unordered_set<std::string> passendenachbarn;

    std::vector<bht::Stop> bahnhofmitgleisinfo = getStopsForTransfer(stopId);
    for (const auto& stop : bahnhofmitgleisinfo)
    {
        if (stop.id != stopId && stop.locationType == LocationType_Stop) 
        {
            passendenachbarn.insert(stop.id);
        }
    }

    auto range = stopzutripsmap.equal_range(stopId); 

    for (auto it = range.first; it != range.second; ++it)
    {
        const std::string& tripId = it->second;

        auto scheduledTripIt = tripsdiescheduledsind.find(tripId);
        if (scheduledTripIt == tripsdiescheduledsind.end())
            continue; 

        const NetworkScheduledTrip* scheduledTrip = scheduledTripIt->second.get();

        auto stopIter = scheduledTrip->at(stopId);
        if (stopIter == scheduledTrip->end())
            continue; 

        ++stopIter;
        if (stopIter != scheduledTrip->end())
        {
            passendenachbarn.insert(stopIter->stopId);
        }
    }

    return passendenachbarn;
}
