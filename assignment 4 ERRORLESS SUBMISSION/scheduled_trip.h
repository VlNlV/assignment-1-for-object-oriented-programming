...
class NetworkScheduledTrip {
    private:
        ...
        Trip ganzetripobjekt;   // speicherung des trip objekts (aus stoptimes)


    public:

    NetworkScheduledTrip(const Trip& ganzetripobjekt, const std::vector<StopTime>& stoptimesvector);  // konstruktor fürs erstellen NetworkScheduledTrip objekts (mit übergabe des ganzen trip objekts und dem vektor der stoptime objekte)

    NetworkScheduledTrip(const NetworkScheduledTrip& anderes);      // kopier konstruktor

    NetworkScheduledTrip(NetworkScheduledTrip&& anderes);           // move konstruktor

  //const std::vector<StopTime>& stoptimesgetter() const;   // getter methode für stoptimes
  
  const Trip& tripIDgetter() const;    // getter methode für trip ID
    ...
    iterator at(const std::string& stopId) const;   // methode die eine stopID innerhalb des stoptimesvectors sucht
};
