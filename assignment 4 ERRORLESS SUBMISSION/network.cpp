...
void Network::StopTimesLoader(std::string path) {       // ähnliches wie oben! siehe "agencyloader"
    ...
    while (reader.next()) {
        ...
        tripzustoptimesmap.insert(std::make_pair(stopTime.tripId, stopTime));       // damit die multimaps die entsprechenden daten bekommen. werden innerhalb des tripsloaders eingesetzt
...
void Network::StopsLoader(std::string path) {       // ähnliches wie oben! siehe "agencyloader"
    ...
    while (reader.next()) {
        ...
        if (stop.locationType == LocationType_Stop && !stop.parentStation.empty())  // für die unterscheidung von station/gleis
        {
            stopzustationsmap.insert(std::make_pair(stop.parentStation, stop.id));
        } 
        else 
        {
            stopzustationsmap.insert(std::make_pair(stop.id, stop.id));
        }
    }
}
...
void Network::TripsLoader(std::string path) {       // ähnliches wie oben! siehe "agencyloader"
    ...
    while (reader.next()) {
        ...
        std::vector<StopTime> chosenstoptimes;  // vektor für die ausgewählten stoptimes

        auto range = tripzustoptimesmap.equal_range(trip.id);   // einteilung von einer tripid und ihre stoptimes
        
        for (auto iterator = range.first; iterator != range.second; ++iterator) // durchgehen der zugehörigen stoptimes des trips
        {
        chosenstoptimes.push_back(iterator->second);    // füge das stoptime in den vektor hinzu
        }
      
        std::sort(chosenstoptimes.begin(), chosenstoptimes.end());  // sortieren der stoptimes (damit die richtige reihenfolge entsteht)
      
        std::shared_ptr<NetworkScheduledTrip> scheduledTrip{new NetworkScheduledTrip(trip, chosenstoptimes)};   // objekt erstellung mit übergabe des trips und dem vektor mit den ausgewählten stoptimes und ihre speicherung als shared_ptr
      
        tripsdiescheduledsind[trip.id] = scheduledTrip; // dies wird unter dem schlüssel "trip.id" gespeichert
    }   
}
...
{
    std::vector<Stop> zugehoerigestops;     // vektor für das endgültige ergebnis

    Stop gefundenerstop = stops.at(stopId); // sucht stopId aus den stops und holt sich dabei das stop-objekt dazu
    
    if (gefundenerstop.locationType == LocationType_Stop && !gefundenerstop.parentStation.empty())  // merkmal abfrage für ein gleis
    {
        gefundenerstop = stops.at(gefundenerstop.parentStation);    // dann finde ihre parentstation in den stops
    }

    auto paar = stopzustationsmap.equal_range(gefundenerstop.id);   // zu jedem stop werden ihre stations zugeordnet
    
    for (auto iterator = paar.first; iterator != paar.second; ++iterator)   // durchgehen der gleise
    {
        zugehoerigestops.push_back(stops[iterator->second]);    // für jedes einzelne gleis wird das stop-objekt hinzugefügt
    }

  return zugehoerigestops;  // returne den endgültigen vektor
}

std::unordered_set<std::string> Network::getNeighbors(const std::string& stopId)    //verlangte methode fürs suchen der nachbarn einer gegebenen stopId
{
   std::unordered_set<std::string> passendenachbarn;    // vektor für die nachbarn

  std::vector<Stop> umstiege = getStopsForTransfer(stopId);     // hol die zugehörigen stop infos (bahnhöfe, gleise) zu der stopId und speichere sie

  for (auto einzelnerumstieg : umstiege)    // geh durch die umstiege (also vektor von oben)
  {
    if (einzelnerumstieg.id != stopId && einzelnerumstieg.locationType == LocationType_Stop) // merkmal abfrage für gleis
    {
      passendenachbarn.insert(einzelnerumstieg.id); // dann füge das gleis hinzu
    }
  }

  auto stoptripspaar = stopzutripsmap.equal_range(stopId);  // zuordnungspaarung von stops zu ihren trips

  for (auto iterator = stoptripspaar.first; iterator != stoptripspaar.second; ++iterator)   // durchgehen der stops
  {
    auto geplantertrip = tripsdiescheduledsind[iterator->second];   // zeigt auf trip-element des paares

    auto geplantertripiterator = geplantertrip->at(stopId); // iterator das nun arauf zeigt

    if (geplantertripiterator == geplantertrip->end()) // wenn er hinter dem ende zeigt (also er nichts findet)
    {
      continue;     // dann gehe weiter
    }
    geplantertripiterator++;    // inkrementiere die position des iterators

    if (geplantertripiterator != geplantertrip->end())  // wenn er was findet
    {
      passendenachbarn.insert(geplantertripiterator->stopId);   // füge es hinzu
    }
  }

  return passendenachbarn;  // returne das endgültige ergebnis
}
...
{
    std::map<std::string, float> entfernung;                  // eine map mit format (string und float), welches die entfernung ("float") zu einer zugehörigen stopID ("string") zuordnet/speichert
    std::map<std::string, std::string> vorgaenger;            // speicherung des vorgänger vom aktuellen knoten (damit wir wissen von vorher wir kamen) im format (string und string) also die stopID von wo wir kamen und wo es uns gebracht hat
    std::set<std::string> nochnichtbesucht;            // speicherung der noch nicht besuchten stops in einem SET
    
    for (auto einzelnerstop : stops)         // durchgehen aller stops (aus stops.txt) und setzen alle einzelnen stops auf unendlich und hinzufügen dieser stops in das "noch-nicht-besucht"-set (so funktioniert halt dijkstra laut https://www.w3schools.com/dsa/dsa_algo_graphs_dijkstra.php)
    {
        entfernung[einzelnerstop.first] = INFINITY;        // da die map ja einen key (.first) und einen value (.second) setzen wir zunächstmal jede stopID auf unendlich bzw. infinity
        
        nochnichtbesucht.insert(einzelnerstop.first);           // dann mit der logik von dijkstra, dass wir alle stops (mit bezeichnung "infinity") in das "noch-nicht-besucht"-set reinpacken (da sie ja nicht besucht worden sind) 
    }
  
    entfernung[fromStopId] = 0;       // wir initialisieren unseren startpunkt (fromStopId), dass er den value (seine entfernung) zunächstmal 0 hat (denn wir fangen erst jetzt an)

    auto weristkleiner = [&entfernung](const std::string& a, const std::string& b)     
    {                                                       // eine lambda funktion, die von zwei punkten auswertet, ob die erste eine kleinere entfernung hat
        float entfernung1 = entfernung[a];
        float entfernung2 = entfernung[b];

        return (entfernung1 < entfernung2);
    };

    while (nochnichtbesucht.size() > 0)        // solange das "noch-nicht-besucht"-set noch was hat, dann gehen wir weiter
    {
        auto amkleinstenfinder = std::min_element(nochnichtbesucht.begin(), nochnichtbesucht.end(), weristkleiner);     // begin zeigt aufs erste element von dem "noch-nicht-besucht"-set und end zeigt HINTER dem letzten davon und mit der sequenz dieser daten wir die lambda-funktion von oben aufgerufen (min_element holt sich dann die kleinste entfernung davon)
        
        std::string naechster = *amkleinstenfinder;      // da min_element einen iterator zurückgibt, arbeiten wir mit " * " (wir speichern hier den nächsten am nähersten stop)

        nochnichtbesucht.erase(naechster);             // da wir also jetzt "da" waren, ist dieser stop jetzt nicht mehr "nicht-besucht" also entfernen wir es aus dem "noch-nicht-besucht"-set

        if (entfernung[naechster] == INFINITY)    // wenn dieser am nächsten näherster stop die entfernung unendlich hat, dann breake aus der schleife
        {
            break;          // verlasse dann die schleife
        }

        for (auto aktuellernachbar : getNeighbors(naechster))      // geh durch die nachbarn von diesem am nächsten nähersten stop (via getNeighbors mit Übergabe von diesem stop)
        {
            float neueentfernung = entfernung[naechster] + 1.0;      // wenn wir vermuten, dass die entfernungen jeweils 1 weg voneinander sind, dann simulieren wir hier, eine stop-bewegung mit der entfernungsänderung mit + 1  (die neue aktuelle entfernung bisher wird jetzt gespeichert)
            
            if (neueentfernung < entfernung[aktuellernachbar])  // wenn unsere aktuelle entfernung kleiner ist als die entfernung eines der nachbarn des am nächsten nähersten stops (logik von dijkstra)
            {
                entfernung[aktuellernachbar] = neueentfernung;      // dann setze die entfernung dieses einen nachbarn des am nächsten nähersten stops mit unserer aktuellen entfernung

                vorgaenger[aktuellernachbar] = naechster;    // der vorgänger wird zu dem stop, wo wir ehemals waren (also dem stop von dem wir die nachbarn geholt haben)
            }
        }
    }

  std::vector<Stop> fertigertravelpath;         // vektor mit dem endgültigen ergebnis (im format von stop-objekten)

  auto reversefinder = vorgaenger.find(toStopId); // jetzt laut dijkstra machen wir den rückkehr weg, um nun den endgültigen kürzesten weg zu finden (wir fangen jetzt am ziel und versuchen den vorgänger davon zu finden)

  while (reversefinder != vorgaenger.end())   // falls das ziel einen einen vorgänger hat (iterator würde auf .end() zeigen, wenn nicht)
  {
    fertigertravelpath.push_back(stops.at(reversefinder->first));   // fügt diesen stop mittels der at-methode in den vektor

    reversefinder = vorgaenger.find(reversefinder->second);       // erweitert sich zum vorgänger (für den rückkehrweg)
  }

  std::reverse(fertigertravelpath.begin(), fertigertravelpath.end());   // da wir jetzt den rückkehrweg gemacht haben, wollen wir nun schließlich den tatsächlichen vorwärtsweg haben also machen wir mittels "reverse" die fahrtrichtung umgekehrt (um richtung start zu stop zu bekommen)
  
  return fertigertravelpath;        // wir printen das ergebnis
}
...
{
   return *tripsdiescheduledsind.at(tripId);    // returnt den trip zur gesuchten tripID
