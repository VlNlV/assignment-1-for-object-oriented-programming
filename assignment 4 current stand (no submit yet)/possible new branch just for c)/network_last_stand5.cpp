...
std::vector<bht::Stop> Network::getStopsForTransfer(const std::string& stopId)  // methode fürs printen der zugehörigen gleise/bahnhöfe zu einer gegebenen ID (plan: wenn bahnhof ID gegeben wird, dann printe diese erneut und ihre zugehörigen gleise und wenn eine einzelne gleis-ID gegeben wird, dann printe ihren zugehörigen bahnhof mit anderen zugehörigen gleisen zu diesem bahnhof also die "geschwister"-gleisen)
{
    ...
    if (ID_identifikator.locationType == 1)     // hier ne abfrage ob die gefiltere ID ein bahnhof ist (via locationType == 1)-check
    {
    ...
    {
        if ( (stop.locationType == 1 && stop.id == IDmitderwirfiltern) || (stop.locationType == 0 && stop.parentStation == IDmitderwirfiltern)) // hier eine fallunterscheidung gemacht, um die bahnhöfe mit locationType == 1 und der ID der filterungs-ID oder die gleise mit locationType == 0 und einer parent station der filterungs-ID zu finden
        {
...
{
    std::unordered_set<std::string> passendenachbarn;   // leerer vektor, der die nachbarn am ende beinhalten wird

    std::vector<bht::Stop> bahnhofmitgleisinfo = getStopsForTransfer(stopId);   // bahnhofmitgleisinfo wird zu den stopIds, also  dem bahnhof mit ihren zugehörigen gleisen (bzw. vice versa)

    for (const auto& aktuellerstop : bahnhofmitgleisinfo)   // es wird durch den bahnhof mit ihren zugehörigen gleisen durchgegangen (einzelner bahnhof/einzelnes gleis in "aktuellerstop" gespeichert)
    {
        const std::string& aktuelleID = aktuellerstop.id;   // die "id" (also stop_id) des aktuell gespeicherten "dings" (bahnhof/gleis) wird in "aktuelleID" gespeichert

        for (size_t index = 0; index < stopTimes.size(); ++index)   // wir gehen jetzt durch die stoptimes (in bezug auf die "länge" von stoptimes-zeilen)
        {
            if(stopTimes[index].stopId == aktuelleID)   // wenn die stopId von einem stoptimes-objekt mit "aktuelleID" übereinstimmt (also ein eintrag von einem bahnhof/gleis auch in stoptimes vorhanden ist)
            {
                const std::string& passendetripID = stopTimes[index].tripId;    // dann speichere die tripId dieses stoptimes-objekts (wo sich "stops" und "stopstimes" übereinstimmen via der "stop_id")

                for(size_t naechsterindex = index + 1; naechsterindex < stopTimes.size(); ++naechsterindex) // durchgehen der stoptimes (erneut) um die benachbarten stationen zu bestimmen (wie gehen jetzt weiter in der stoptimes-datei)
                {
                    if (stopTimes[naechsterindex].tripId != passendetripID)     // wenn die nächste index einer tripId nicht zu unserer "passendetripID" übereinstimmt (also die station ist anscheinend die letzte, hat also keine nächsten stationen)
                    {
                        break;                  // dann breake und verlasse die schleife
                    }
                    if (stops.find(stopTimes[naechsterindex].stopId) != stops.end()) {
                        passendenachbarn.insert(stopTimes[naechsterindex].stopId);  // wenn allerdings die nächste zeile in stoptimes unter "tripId" unserer "passendetripID" entspricht, dann füge ihre stopId in den vektor 
                        break;  // und breake sofort danach denn wir wollen ja nur den "nächsten" stop
                    } 
                }
            }
        }
        for (const auto& otherstop : bahnhofmitgleisinfo)   // weiteres durchgehen, um zu checken ob wir innerhalb einer station noch via gleis/bahnhof umsteigen können
        {
            if(otherstop.id != aktuelleID)  // wenn ein einzelnes gleis/einzelner bahnhof nicht "aktuelleID" (von oben in der aktuellen for-schleifen-iteration ) entspricht
            {
                if (stops.find(otherstop.id) != stops.end())
                {
                    passendenachbarn.insert(otherstop.id);  // dann füge diesen stop mittels ihrer ID in den vektor ebenfalls
                }
            }
        }
    }
    return passendenachbarn;        // returne den vektor
}
...
std::vector<bht::Stop> Network::getTravelPath(const std::string& fromStopId, const std::string& toStopId)   // verlangt methode für die berechnung des kürzesten weg von start bis ziel
{
    ...
    std::set<std::string> bishernichtbesucht;   // ein set für die speicherung von stopIds die unbesucht sind

    for (const auto& [stopId, stop] : stops)    // durchgehen aller stops (einteilung in ihre stopID und ihr rest des stop-objekts)
    {
        abstand[stopId] = INFINITY;     // setzung des abstandes für jeden stop (mittels ihrer ID) zu unendlich (so halt funktioniert der dijkstra algorithmus, denn er setzt alles auf infinity)

        bishernichtbesucht.insert(stopId);   // alle stops werden dann einzeln in die "unbesucht"-set reingesteckt
    }
    ...
    while (!bishernichtbesucht.empty())         // solange es stops in "bishernichtbesucht" existieren
    {
        auto kleinster = std::min_element(bishernichtbesucht.begin(), bishernichtbesucht.end(), abstandsvergleicher);   // unter allen stops wird derjenige mit dem kleinsten abstand herausgefiltert

        if (kleinster == bishernichtbesucht.end())  // wenn überhaupt etwas existiert, der ein kleinster abstand sein kann
        {
            break;          // dann breake
        }
        std::string stelle_aktuell = *kleinster;        // der stop mit dem kleinsten abstand wird als aktuelle stelle gespeichert

        bishernichtbesucht.erase(kleinster);            // da wir ihn jetzt besuchen, wird er jetzt aus dem "bishernichtbesucht"-set entfernt

        if (abstand[stelle_aktuell] == INFINITY)        // wenn unsere aktuelle stelle einen unendlichen abstand hat
        {
            break;      // dann breake
        }

        for (const auto& einzelnernachbar : getNeighbors(stelle_aktuell))       // geh durch alle nachbarn der aktuellen stelle durch
        {
            if (bishernichtbesucht.find(einzelnernachbar) == bishernichtbesucht.end())      // wenn der nachbar besucht ist
            {
            ...
            if (geupdateterabstand < abstand[einzelnernachbar])         // wenn der neue abstand kürzer ist als den abstand, den wir zurzeit als kürzesten betrachten
            {
                abstand[einzelnernachbar] = geupdateterabstand;     // dann wird der neu kürzesten abstand zu diesem abstand

                vorgaenger[einzelnernachbar] = stelle_aktuell;      // die stelle wo wir waren, wird zum vorgänger (um vorherigen weg zu speichern)
            }
        }
    }
    ...
    while (reversegeher != vorgaenger.end())        //  solange wir vom ziel nicht zum start gelangen sind
    {
        ...
        aktuell_for_reverse = reversegeher->second;     // der vorgänger wird auf die aktuelle ID gesetzt
        
        reversegeher = vorgaenger.find(aktuell_for_reverse);    // für den nächsten vorgänger wird reversegeher geupdatetet
    }
