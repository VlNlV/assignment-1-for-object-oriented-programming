...
std::vector<bht::Stop> Network::getTravelPath(const std::string& fromStopId, const std::string& toStopId)   // verlangt methode für die berechnung des kürzesten weg von start bis ziel
{
   ...
   {                                                                            // lambda-funktion (selbstrecherche!!!) : hilft uns wenn wir zwei abstände vergleichen wollen
    ...
    {
        auto it = stops.find(aktuell_for_reverse);  // auf das ziel wird in stops gezeigt
        
        if(it == stops.end())   // wenn kein ziel in stops existiert
        {
            return{};       // dann returne was leeres
        }
        createdpath.push_back(it->second);   // wenn die aktuelle position tatsächlich existiert, dann füge sie in den end-vektor ein
        ...
        auto anderer_it = stops.find(fromStopId);   // auf start aus den stops wird gezeigt
        
        if(anderer_it == stops.end())   // wenn start nicht existiert
        {   
            return{};   // dann returne was leeres
        }
        
        createdpath.push_back(anderer_it->second);  // das ganze stop-objekt(via second) wird in den end-vektor gesteckt
