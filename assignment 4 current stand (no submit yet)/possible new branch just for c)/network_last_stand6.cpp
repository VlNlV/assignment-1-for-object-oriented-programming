...
std::unordered_set<std::string> Network::getNeighbors(const std::string& stopId)    //verlangte methode fürs suchen der nachbarn einer gegebenen stopId
{
    ...
    std::unordered_set<std::string> bereitsbearbeitet;
    ...
    for (const auto& aktuellerstop : bahnhofmitgleisinfo)   // es wird durch den bahnhof mit ihren zugehörigen gleisen durchgegangen (einzelner bahnhof/einzelnes gleis in "aktuellerstop" gespeichert)
    {
        ...
        if (bereitsbearbeitet.count(aktuelleID))
        {
            continue;
        }
        bereitsbearbeitet.insert(aktuelleID);
    ...
    for (const auto& otherstop : bahnhofmitgleisinfo)   // weiteres durchgehen, um zu checken ob wir innerhalb einer station noch via gleis/bahnhof umsteigen können
        {
            if (otherstop.id != stopId && stops.find(otherstop.id) != stops.end())
            {
                passendenachbarn.insert(otherstop.id);
            }
        }
